					...
					$.each(json.results,function(i,item) {
						if(e==1 || (i<numMSG && item.id>ultID)){
							...
							if (item.text != "undefined") {
								...
								if(fromID=="image") mHTML="<a href='http://www.twitter.com/"+item.from_user+"'><img src='"+item.profile_imageurl+"' alt='"+item.from_user+"' class='juitterAvatar' /></a> "+$.Juitter.textFormat(tweet)+" <span class='time'>"+item.created_at+"</span> <a href='" + link + "' class='JRM' "+openLink+">"+readMore+"</a>";
